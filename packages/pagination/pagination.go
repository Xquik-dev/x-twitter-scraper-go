...
type CursorPageLegacy[T any] struct {
	Items      []T    `json:"items"`
...
// Returns the unmodified JSON received from the API
func (r CursorPageLegacy[T]) RawJSON() string { return r.JSON.raw }
func (r *CursorPageLegacy[T]) UnmarshalJSON(data []byte) error {
	return apijson.UnmarshalRoot(data, r)
}
...
// there is no next page, this function will return a 'nil' for the page value, but
// will not return an error
func (r *CursorPageLegacy[T]) GetNextPage() (res *CursorPageLegacy[T], err error) {
	if len(r.Items) == 0 {
		return nil, nil
	}
	...
	cfg := r.cfg.Clone(r.cfg.Context)
	err = cfg.Apply(option.WithQuery("afterCursor", next))
	if err != nil {
		return nil, err
	}
	var raw *http.Response
...
func (r *CursorPageLegacy[T]) SetPageConfig(cfg *requestconfig.RequestConfig, res *http.Response) {
	if r == nil {
		r = &CursorPageLegacy[T]{}
	}
	r.cfg = cfg
	r.res = res
}

type CursorPageLegacyAutoPager[T any] struct {
	page *CursorPageLegacy[T]
	cur  T
	idx  int
	run  int
	err  error
...
func NewCursorPageLegacyAutoPager[T any](page *CursorPageLegacy[T], err error) *CursorPageLegacyAutoPager[T] {
	return &CursorPageLegacyAutoPager[T]{
		page: page,
		err:  err,
	}
}

func (r *CursorPageLegacyAutoPager[T]) Next() bool {
...
func (r *CursorPageLegacyAutoPager[T]) Current() T {
	return r.cur
}

func (r *CursorPageLegacyAutoPager[T]) Err() error {
	return r.err
}

func (r *CursorPageLegacyAutoPager[T]) Index() int {
	return r.run
}
